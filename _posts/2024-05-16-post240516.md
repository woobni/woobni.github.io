...
- 모니터링 스택들을 쿠버네티스 클러스터 위에 구축하여 시스템 리소스 및 애플리케이션 사용량 모니터링을 가능하게 함.
...
- 기본적으로 같은 네임스페이스의 ServiceMonitor로 서비스를 디스커버리 하기 때문에 이 기본 동작을 Disable하고 다른 네임스페이스의 ServiceMonitor를 탐색할 수 있도록 설정
- 추후 테넌트별 리소스/메트릭 구분을 위해 라벨 설정
